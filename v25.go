...
//Determines if person belongs in TM[0][1] i.e. cycle 1, dz state 0. WORKS!
	if (person1[0][cycle-1]==1 && r[cycle]<=sumTM[0][0]) {
			person1[0][cycle] = 1
			}
...
//Determines if person belongs in TM[1][1] i.e. cycle 1, dz state 1
	for i:=0; i<numDzStates; i++ {	
		if (person1[0][cycle-1]==1 && r[cycle]>sumTM[0][0] && r[cycle]<=sumTM[0][1]) {
			person1[1][cycle] = 1
			}
