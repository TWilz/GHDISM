...
//v32 scalable to any number of dz states AND any number of cycles
package main
...
//2nd parameter is # cycles person goes through
	const numCycles = 20
	fmt.Println("Number of Cycles:",numCycles,"\n\n")//db
...
// whatever number goes inside of the Seed function will determine random #'s. can use time to randomize
	rand.Seed(10)
	var random [numCycles]float32
	for o :=0; o<numCycles; o++ {
		random[o] = rand.Float32()
		}
	fmt.Println("The random numbers are:\t", random, "\n")	


//Here you can set where the person starts
//random[0]=.99 //db
...
if (random[0] <= verticalSumTM[0][0]) {
	person1[0][0]=1
	}
for r:=1; r<numDzStates; r++ {
	if (random[0] > verticalSumTM[r-1][0] && random[0] <= verticalSumTM[r][0]) {
		person1[r][0]=1
	}
}
/*
...
//Determines if person belongs in TM[0][1] i.e. cycle 1, dz state 0. WORKS!!
	for i:=0; i<numDzStates; i++ {
	if (person1[i][cycle-1]==1 && random[cycle]<=sumTM[i][0]) {
			person1[0][cycle] = 1
			}
	}
//Determine if person belongs in TM[x][1] i.e. cycle 1, dz state 1 through numDzStates. WORKS!!
	for o:=1; o<numDzStates; o++ {
		for j:=0; j<numDzStates; j++ {
			if (person1[j][cycle-1]==1 && random[cycle]>sumTM[j][o-1] && random[cycle]<=sumTM[j][o]) {
				person1[o][cycle] = 1
				}
		}
	}
...
//Prints person's path in readable format
	fmt.Println("The individual's path is:")
	fmt.Println("\t\tCycle")
	fmt.Print("\t")
