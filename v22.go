...
v22 cycle 1, row 1 works with for loop rather than if statements
package main
...
//determines person's path through dz states
	...
	r[c]=0.6
	//Determines if person belongs in TM[0][1] i.e. cycle 1, dz state 0
	...
	for run:=1;run<numDzStates;run++ {
		if (person1[run][c-1]==1 && r[c]>sumTM[run-1][0] && r[c]<=sumTM[run][0]) {
			person1[0][c]=1
			}
		}
