...
func printUtilities() {
//type stateSeries [numberOfPeople][numDzs][maxNumCycles]int
	fmt.Println("\n\nAfter modeling the diseases and interventions, the utilities of the people are:")
	for per:=0;per<numberOfPeople;per++ {
		fmt.Println("\nFor person",per,":")
			for dz:=0;dz<numDzs;dz++{
...
//				fmt.Println("state[0][0][9]:",state[0][0][9])
				fmt.Println("disease ",dz,", utility on last cycle is",u[dz][state[per][dz][(numCycles[dz]-1)]])
