...
//GHDISM17 put in user input options for running the model. note that the model can only be run 1x and then needs to be restarted
//			and the intervention can only be added once and then the model needs to be restarted
...
//Declaration of global variables
	...
	const numberOfPeople = 3 //2 being 0th n 1st person
	const cyclesPerYear = 1.0
...
//				}
	...
	fmt.Println("With the diseases and interventions in this run of the model, the cost per average QALY at the end of ",maxNumCycles," is: USD$",float32(totalExpense/averageQALYforAllPeople))
}
